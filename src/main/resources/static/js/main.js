	function save(formData,url) {
		$.ajax({
			...
			success : function(result) {
				Materialize.toast('<i class="material-icons">done</i>User saved.', 2000,'green rounded');
	...
	function remove(id,url){
		$.ajax({
			...
			type : "DELETE",
			success : function(result) {
				Materialize.toast('<i class="material-icons">done</i>User deleleted.', 2000,'green rounded');
				setTimeout(function() {location.reload();}, 2500);
			},
			error : function(xhr, resp, text) {
