...
public class AdminServiceSimpleTest {
  ...
  public void testGetAllPosts_withEmptyPaging_shouldPushCorrect() throws Exception {
    ...
    PostsPageDTO result = service.getPostsByQuery(PostQuery.all().size(6).build());
  ...
  public void testGetPostsByStatus_withCorrectPaging_shouldPushCorrect() throws Exception {
    ...
    PostsPageDTO result =
        service.getPostsByQuery(PostQuery.all().status(PostStatus.PUBLIC).page(3).size(4).build());
    ...
    assertEquals("should push correct post status", PostStatus.PUBLIC.toString(),
        catchStatus.getValue());

    verify(client, times(1)).getPostsByStatus(matches(PostStatus.PUBLIC.toString()), anyInt(),
        anyInt());
  }
  ...
  public void testGetPostsByUser_withPublicStatus_shouldPushCorrectApiCall() throws Exception {
    ...
    PostsPageDTO posts = new PostsPageDTO();
    when(client.getPostsByUserAndStatus(catchUser.capture(), catchStatus.capture(),
        catchPage.capture(), catchSize.capture())).thenReturn(posts);

    PostsPageDTO result = service.getPostsByQuery(PostQuery.filter(PostFilter.USER).id(34l)
        .status(PostStatus.PUBLIC).page(3).size(4).build());
    ...
    verify(client, times(1)).getPostsByUserAndStatus(anyLong(),
        matches(PostStatus.PUBLIC.toString()), anyInt(), anyInt());
  }
  ...
  public void testGetPostsByUser_withAllStatus_shouldPushCorrectApiCall() throws Exception {
    ...
    ArgumentCaptor<Long> catchUser = ArgumentCaptor.forClass(Long.class);
    PostsPageDTO posts = new PostsPageDTO();
    when(client.getPostsByUser(catchUser.capture(), catchPage.capture(), catchSize.capture()))
        .thenReturn(posts);

    PostsPageDTO result = service.getPostsByQuery(
        PostQuery.filter(PostFilter.USER).id(34l).status(PostStatus.ALL).page(3).size(4).build());
  ...
  public void testGetPostsByTag_withPublicStatus_shouldPushCorrectApiCall() throws Exception {
    ...
    PostsPageDTO posts = new PostsPageDTO();
    when(client.getPostsByTagAndStatus(catchTag.capture(), catchStatus.capture(),
        catchPage.capture(), catchSize.capture())).thenReturn(posts);

    PostsPageDTO result = service.getPostsByQuery(
        PostQuery.filter(PostFilter.TAG).id(34l).status(PostStatus.PUBLIC).page(3).size(4).build());
    ...
    verify(client, times(1)).getPostsByTagAndStatus(anyLong(),
        matches(PostStatus.PUBLIC.toString()), anyInt(), anyInt());
  }
  ...
  public void testGetPostsByTag_withAllStatus_shouldPushCorrectApiCall() throws Exception {
    ...
    ArgumentCaptor<Long> catchTag = ArgumentCaptor.forClass(Long.class);
    PostsPageDTO posts = new PostsPageDTO();
    when(client.getPostsByTag(catchTag.capture(), catchPage.capture(), catchSize.capture()))
        .thenReturn(posts);

    PostsPageDTO result = service.getPostsByQuery(
        PostQuery.filter(PostFilter.TAG).id(34l).status(PostStatus.ALL).page(3).size(4).build());
  ...
  public void testGetPostsByCategory_withPublicStatus_shouldPushCorrectApiCall() throws Exception {
    ...
    PostsPageDTO posts = new PostsPageDTO();
    when(client.getPostsByCategoryAndStatus(catchCategory.capture(), catchStatus.capture(),
        catchPage.capture(), catchSize.capture())).thenReturn(posts);

    PostsPageDTO result = service.getPostsByQuery(PostQuery.filter(PostFilter.CATEGORY).id(34l)
        .status(PostStatus.PUBLIC).page(3).size(4).build());
    ...
    verify(client, times(1)).getPostsByCategoryAndStatus(anyLong(),
        matches(PostStatus.PUBLIC.toString()), anyInt(), anyInt());
  }
  ...
  public void testGetPostsByCategory_withAllStatus_shouldPushCorrectApiCall() throws Exception {
    ...
    ArgumentCaptor<Long> catchCategory = ArgumentCaptor.forClass(Long.class);
    PostsPageDTO posts = new PostsPageDTO();
    when(client.getPostsByCategory(catchCategory.capture(), catchPage.capture(),
        catchSize.capture())).thenReturn(posts);

    PostsPageDTO result = service.getPostsByQuery(PostQuery.filter(PostFilter.CATEGORY).id(34l)
        .status(PostStatus.ALL).page(3).size(4).build());
