...
public class AllKindsOfClientErrorsTest {
	...
	public void test_withStatusNotFound_shouldThrowEntityIsNotExistingException() throws Exception {
		...
		spongeMock.stubFor(get(urlEqualTo("/api/v1/blog")).willReturn(aResponse().withStatus(404).withHeader("Content-Type", "application/json").withBody(toJson(error))));
	...
	public void test_withConflict_shouldThrowEntityConflictException() throws Exception {
		...
		spongeMock.stubFor(post(urlEqualTo("/api/v1/posts")).willReturn(aResponse().withStatus(409).withHeader("Content-Type", "application/json").withBody(toJson(error))));
	...
	public void test_withBadRequest_shouldThrowDtoValidationFailedException() throws Exception {
		...
		spongeMock.stubFor(put(urlEqualTo("/api/v1/posts")).willReturn(aResponse().withStatus(400).withHeader("Content-Type", "application/json").withBody(toJson(error))));
		service.updatePost(new PostDTO());
	}
	
	@Ignore
	@Test
	public void test_withBadRequestWithoutBody_shouldThrowDtoValidationFailedException() throws Exception {
		expectedEx.expect(DtoValidationFailedException.class);
		
		spongeMock.stubFor(put(urlEqualTo("/api/v1/posts")).willReturn(aResponse().withStatus(400).withHeader("Content-Type", "application/json")));
		service.updatePost(new PostDTO());
	}
	...
	public void test_withServiceIsLazy_shouldThrowDtoValidationFailedException() throws Exception {
		...
		spongeMock.stubFor(put(urlEqualTo("/api/v1/posts")).willReturn(aResponse().withStatus(500).withHeader("Content-Type", "application/json").withBody(toJson(error))));
