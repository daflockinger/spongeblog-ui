...
public class BlogServiceSimpleTest {
  ...
  public void testGetCategories_shouldMapCorrectly() throws Exception {
    ...
    Category catResult1 =
        new Category("uber category", 2l, ImmutableList.of(subResult1, subResult2));
    ...
    assertEquals("validate first child id", subResult1.getId(),
        results.get(0).getChildren().get(0).getId());
    assertEquals("validate first child name", subResult1.getName(),
        results.get(0).getChildren().get(0).getName());
    assertEquals("validate second child id", subResult2.getId(),
        results.get(0).getChildren().get(1).getId());
    assertEquals("validate second child name", subResult2.getName(),
        results.get(0).getChildren().get(1).getName());
  ...
  public void testGetPost_withValidPostId_shouldMapCorrectly() throws Exception {
    ...
    assertEquals("validate part-content",
        ...
        result.getContent());
    assertEquals("validate title",
        "Fish don't fry in the kitchen and beans don't burn on the grill.", result.getTitle());
  ...
  public void testGetPostsByCategory_withPublicStatus_shouldMapCorrectly() throws Exception {
    ...
    assertEquals("validate title",
        "Fish don't fry in the kitchen and beans don't burn on the grill.", resultPost.getTitle());
    assertTrue("validate first tag",
